...
export async function POST(
  ...
  try {
    ...
    if (!isFull) {
      ...
      // High-priority player bumps the lowest-priority confirmed player
      // Use service client to modify another user's registration
      const admin = await createServiceClient();

      // Find the lowest-priority confirmed player to bump
      // Priority order for bumping: low first, then normal — never bump another high
      ...
      // Sort: low priority first, then normal, then high — within same priority, latest signup first
      ...
      const sorted = (confirmed ?? []).sort((a: any, b: any) => {
        ...
        // Same priority: bump the one who signed up latest
